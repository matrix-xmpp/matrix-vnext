...
namespace Matrix.Xmpp.Auth
{
    ...
    public class Auth : XmppXElement
    {
        ...
        internal void SetAuthDigest(string username, string password, string streamId)
        {
            Username = username;
            Digest = Core.Crypt.Hash.Sha1HashHex(streamId + password);
        }
        
        internal void SetAuthPlain(string username, string password)
        {
            Username = username;
            Password = password;
        }
