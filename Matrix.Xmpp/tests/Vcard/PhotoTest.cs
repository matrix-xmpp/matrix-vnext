using System;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using Matrix.Xml;
using Matrix.Xmpp.Vcard;
using Shouldly;

namespace Test.Xmpp.Vcard
{
    [TestClass]
    public class PhotoTest
    {
        private const string xml1 = @"<PHOTO xmlns='vcard-temp'>
<TYPE>image/jpeg</TYPE>
<BINVAL>

ocUxKLzi8Tpr6l2TAf//Z</BINVAL>
</PHOTO>";

        private const string xml2 = @"<PHOTO xmlns='vcard-temp'>
<TYPE>image/jpeg</TYPE>
<BINVAL></BINVAL>
</PHOTO>";

        private const string xml3 = @"<PHOTO xmlns='vcard-temp'>
<TYPE>image/jpeg</TYPE>
</PHOTO>";

        [TestMethod]
        public void Test1()
        {
            var xmpp1 = XmppXElement.LoadXml(xml1);
            xmpp1.ShouldBeOfType<Photo>();
            
            var photo = xmpp1 as Photo;
            if (photo != null)
            {
                byte[] b1 = photo.Binval;

                byte[] bcheck = Convert.FromBase64String(@"
ocUxKLzi8Tpr6l2TAf//Z");

                b1.ShouldBe(bcheck);
            }

            var xmpp2 = XmppXElement.LoadXml(xml2);
            Assert.AreEqual(true, xmpp2 is Photo);
            var photo2 = xmpp2 as Photo;
            if (photo2 != null)
            {
                byte[] b2 = photo2.Binval;
                b2.ShouldBe(new byte[0]);
            }

            var xmpp3 = XmppXElement.LoadXml(xml3);
            Assert.AreEqual(true, xmpp3 is Photo);
            var photo3 = xmpp3 as Photo;
            if (photo3 != null)
            {
                byte[] b3 = photo3.Binval;
                Assert.AreEqual(b3, null);
            }
        }
    }
}





